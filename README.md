...
このゲームは **インターネット越しでもプレイ可能** です。以下の手順でサーバーを公開できます：

#### 1. サーバー側の設定

**グローバルIPアドレスを確認:**
- ブラウザで「[確認くん](https://www.ugtop.com/spill.shtml)」や「[What is my IP](https://www.whatismyip.com/)」にアクセス
- 表示されるIPアドレスをメモ（例: `203.0.113.10`）

**ルーターでポートフォワーディング設定:**
1. ルーターの管理画面にアクセス（通常 `192.168.1.1` または `192.168.0.1`）
2. 「ポートフォワーディング」「仮想サーバー」「NAT設定」などのメニューを探す
3. 以下を設定：
   - **外部ポート**: ゲームで使用するポート（例: `50100`）
   - **内部ポート**: 同じポート番号（例: `50100`）
   - **内部IPアドレス**: サーバーPCのローカルIP（例: `192.168.1.10`）
   - **プロトコル**: TCP/UDP 両方
4. 設定を保存

**ファイアウォールの設定:**
- Windows/Linux のファイアウォールで該当ポートを許可（上記「ポート番号について」参照）

#### 2. クライアント側の接続

サーバーのグローバルIPアドレスとポート番号を使って接続：
```bash
HeavensFortressClient 203.0.113.10 50100
```

**注意事項:**
- グローバルIPアドレスは変動する場合があります（動的IP）
- 固定IPサービスやDDNS（Dynamic DNS）を利用すると便利です
- セキュリティのため、プレイ後はポートフォワーディングを無効にすることを推奨します
